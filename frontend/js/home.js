...
function placeholderSVG() {
    return `<div class="card-foto-placeholder"><svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="1.2"><rect x="4" y="10" width="40" height="30" rx="4"/><circle cx="24" cy="23" r="7"/><circle cx="36" cy="16" r="2" fill="currentColor"/></svg></div>`;
}
function criarCard(a) {
    const st  = LABEL_STATUS[a.status]||{texto:a.status,css:''};
    const esp = LABEL_ESPECIE[a.especie]||a.especie;
    const foto = a.foto_url ? `<img class="card-foto" src="${a.foto_url}" alt="${a.nome}" onerror="this.parentNode.innerHTML='${placeholderSVG().replace(/'/g,"\\'")}'">` : placeholderSVG();
    const card = document.createElement('div');
    card.className='card-animal';
    card.onclick=()=>window.location.href=`animal.html?id=${a.id}`;
    card.innerHTML=`${foto}<div class="card-body"><h3>${a.nome}</h3><p>${esp}${a.raca?' · '+a.raca:''}</p><span class="badge-card ${st.css}">${st.texto}</span></div>`;
    return card;
}
function renderizarSecao(faixa, lista) {
    ...
    grid.innerHTML=''; badge.textContent=lista.length;
