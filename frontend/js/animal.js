...
function render(animal) {
    ...
    const enfHtml = animal.enfermidade
        ? `<div class="enf-card"><p class="enf-label">Condição de saúde</p><p class="enf-text">${animal.enfermidade}</p></div>`
        : '';
...
if (!animal) {
    document.getElementById('conteudo').innerHTML = '<p style="text-align:center;margin-top:3rem;color:#C0392B;">Animal não encontrado.</p>';
} else {
    render(animal);
}
