...
document.getElementById('inputFoto').addEventListener('change',function(){
    ...
    r.onload=e=>{fotoDataUrl=e.target.result;const p=document.getElementById('previewFoto');p.src=fotoDataUrl;p.style.display='block';document.querySelector('.upload-label').textContent=file.name;};
...
document.getElementById('btnCadastrar').addEventListener('click',()=>{
    const nome=document.getElementById('nome').value.trim(),especie=document.getElementById('especie').value,
          raca=document.getElementById('raca').value.trim(),dataNasc=document.getElementById('dataNasc').value,
          descricao=document.getElementById('descricao').value.trim(),descEnf=document.getElementById('descEnfermidade').value.trim(),
          erroGeral=document.getElementById('erroGeral'),sucesso=document.getElementById('sucesso');
    ['erro-nome','erro-especie','erro-enfermidade','erro-desc-enfermidade','erro-descricao'].forEach(esconderErro);
    erroGeral.classList.add('hidden');sucesso.classList.add('hidden');
    let ok=true;
    ...
    if(!ok)return;
    cadastrarAnimal({nome,especie,raca:raca||null,data_nascimento:dataNasc||null,ong_id:sess.id,ong_nome:sess.nome,enfermidade:enfermidadeVal==='sim'?descEnf:null,descricao,foto_url:fotoDataUrl||null});
    sucesso.textContent='Animal cadastrado! Redirecionando...';sucesso.classList.remove('hidden');
